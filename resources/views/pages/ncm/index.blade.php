...
@section('page_content')
	...
	<!-- Upmenu form -->
	<?php $route_importacao = route($Page->link.'.importar');  ?>
						...
						<table border="0" class="table table-hover">
							...
							@foreach ($Buscas as $selecao)
								<tr>
									...
									<td>{{$selecao->codigo}}</td>
									<td>{{$selecao->descricao}}</td>
									<td>
										<a class="btn btn-default btn-xs"
										   href="{{route($Page->link.'.show',$selecao->idncm)}}">
											<i class="fa fa-edit"></i> Visualizar / Editar</a>
										<a class="btn btn-danger btn-xs"
										   data-nome="{{$selecao->descricao}}"
										   data-href="{{route($Page->link.'.destroy',$selecao->idncm)}}"
										   data-toggle="modal"
