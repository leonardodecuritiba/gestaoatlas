				...
				<div class="row">
					<div class="col-md-12 col-sm-12 col-xs-12 animated fadeInDown">
						<table border="0" class="table table-hover">
							...
							<tbody>
							@foreach ($Buscas as $tabela_preco)
								<tr>
									<td>{{$tabela_preco->created_at}}</td>
									<td>{{$tabela_preco->descricao}}</td>
									<td>
										<a class="btn btn-default btn-xs"
												href="{{route($Page->link.'.show',$tabela_preco->idtabela_preco)}}">
											<i class="fa fa-edit"></i> Visualizar / Editar</a>
										<button class="btn btn-danger btn-xs"
												data-nome="Grupo: {{$tabela_preco->descricao}}"
												data-href="{{route($Page->link.'.destroy',$tabela_preco->idtabela_preco)}}"
												data-toggle="modal"
