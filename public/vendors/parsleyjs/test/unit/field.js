...
append('<input type="email" id="element" />');
var parsleyField = $('#element').parsley();
expect(parsleyField.constraints.length).to.be(1);
...
append('<input type="text" id="element" value="hola" data-parsley-minlength="5" />');
var parsleyField = $('#element').parsley();
...
append('<input type="email" pattern="\\w+" id="element" required min="5" max="100" minlength="1" maxlength="3" />');
var parsleyField = $('#element').parsley();
...
append('<input type="date" id="element" max="2000-01-02" min="1999-01-02" value="1998-12-30" />');
var parsleyField = $('#element').parsley();
...
append('<input type="text" id="element" value="" />');
var parsleyField = $('#element').parsley()
    .addConstraint('required', true);
expect(parsleyField.isValid()).to.be(false);
$('#element').val('foo');
...
append('<input type="text" id="element" value="foo" />');
var parsleyField = $('#element').parsley()
...
append('<input type="email" data-parsley-required id="element" />');
...
append('<input type="email" pattern="[A-F][0-9]{5}" required id="element" />');
var parsleyField = $('#element').parsley();
expect(parsleyField.isValid()).to.be(false);
...
append('<input type="email" required id="element" value="foo@bar.baz" />');
$('#element').psly()
...
append('<input type="email" data-parsley-rangelength="[5, 10]" id="element" />');
...
expectWarning(() = > {
    expect($('<input type="email" value="foo">').parsley().isValid(true, '')
).
to.be(false);
})
;
...
append('<input type="text" id="element" value=" foo    bar " />');
...
expectWarning(() = > {
    expect($('#element').parsley().getValue()
).
to.be('foo');
})
;
...
append('<input data-parsley-debounce="100" value="x" required data-parsley-trigger="validatenow" id="element"/>');
$('#element').parsley()
    .on('field:error', () = > done()
)
.
...
append('<form id="element" data-parsley-required>' +
...
append('<input type="text" id="element" required value/>');
