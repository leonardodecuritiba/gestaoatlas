...
append('<input type="text" data-parsley-remote="http://foo.bar" id="element" required name="element" value="foo" />');
...
append('<input type="text" data-parsley-remote="http://foo.bar" id="element" data-parsley-remote-reverse="true" required name="element" value="baz" />');
var parsleyInstance = $('#element').parsley();
...
append('<input type="text" data-parsley-remote="http://foo.bar" id="element" data-parsley-remote-options=\'{ "type": "POST", "data": {"foo": "bar"} }\' required name="element" value="baz" />');
var parsleyInstance = $('#element').parsley();

stubAjax(200);
parsleyInstance.whenValid()
    .done(() = > {
    expect($.ajax.calledWithMatch({type: 'POST'})
).
to.be(true);
...
append('<input type="text" data-parsley-remote id="element" data-parsley-remote-validator="mycustom" required name="element" value="foobar" />');
var parsleyInstance = $('#element').parsley();
...
parsleyInstance.whenValid()
    .fail(() = > {
    expect($.ajax.calledWithMatch({url: 'http://foobar.baz'})
).
to.be(true);
done();
})
...
parsley().addAsyncValidator('instancetest', $.noop, '');
})
;
...
parsleyInstance.whenValid()
    .done(() = > {
    expect($.ajax.calledWithMatch({url: 'http://parsleyjs.org/test/foobar'})
).
to.be(true);
...
parsleyInstance.whenValid()
    .done(() = > {
    expect($.ajax.calledWithMatch({url: 'http://parsleyjs.org/thisisrest/foo%20bar'})
).
to.be(true);
